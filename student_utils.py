...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    df = pd.merge(df,ndc_df,on='NDC_Code')
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    sample_size_train = round(total_values * (1 - test_per - val_per))
    sample_size_val = round(total_values * (1 - test_per))
    train = df[df[patient_key].isin(unique_values[:sample_size_train])].reset_index(drop=True)
    validation = df[df[patient_key].isin(unique_values[sample_size_train:sample_size_val])].reset_index(drop=True)
    test = df[df[patient_key].isin(unique_values[sample_size_val:])].reset_index(drop=True)
...
def get_student_binary_prediction(df, col):
    ...
    '''
    student_binary_prediction = df[col].apply(lambda x: 1 if x>=4 else 0)
